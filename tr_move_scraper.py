#!/usr/bin/env python3
"""
TR Move Scraper Tool for Pokémon Set Builder

This script scrapes TR move data from PokemonDB for specified Pokémon
from Generation 8 (Sword/Shield) that don't have Gen 9 movesets,
and adds them to the modern_tm_learnset.json file as if they were TMs.
"""
...
def has_gen9_moves(pokemon_name):
    ...
    try:
        ...
        # First, check if there's a "Pokémon not in this generation" message
        not_in_gen_message = soup.find('p', class_='alert alert-error')
        if not_in_gen_message and "not in this generation" in not_in_gen_message.text:
            print(f"{pokemon_name} is not in Generation 9")
            return False
        
        # Check if there are any move sections with actual moves
        # This will find sections like "Moves learnt by level up", "Moves learnt by TM", etc.
        move_sections = []
        for h3 in soup.find_all('h3'):
            if "Moves learnt by" in h3.text:
                # Check if this section has a data table with moves
                section = h3.find_next('div', class_='infocard-list-evo') or h3.find_next('table', class_='data-table')
                if section:
        ...
        # Check if any move section has actual moves
        has_moves = False
        for section in move_sections:
            # Look for move links in the section
            move_links = section.find_all('a', href=lambda href: href and href.startswith('/move/'))
            if move_links:
                has_moves = True
                break
        ...
        if has_moves:
            ...
            print(f"No moves found for {pokemon_name} in Generation 9")
            return False
        
    except Exception as e:
        print(f"Error checking Gen 9 moves for {pokemon_name}: {str(e)}")
        return False  # Assume no Gen 9 moves if there's an error
...
def update_tm_learnset(pokemon_species, tr_moves, output_file='data/modern_tm_learnset.json'):
    ...
    if pokemon_species in learnset_data:
        ...
        print(f"Added {len(new_moves)} TR moves to existing TM moves for {pokemon_species}")
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape TR moves from PokemonDB Gen 8 for Pokémon not in Gen 9')
    parser.add_argument('pokemon', nargs='+', help='Pokémon name(s) to scrape (e.g., klinklang rotom-wash)')
    parser.add_argument('--species', action='store_true', help='Names are already in SPECIES_NAME format')
    parser.add_argument('--skip-gen9-check', action='store_true', 
                        help='Skip checking if Pokémon is in Gen 9 (scrape TR moves regardless)')
    ...
    for pokemon in args.pokemon:
        ...
        if not args.skip_gen9_check and has_gen9_moves(pokemon_name):
            print(f"Skipping {pokemon_name} as it has Gen 9 moves")
            continue
