#!/usr/bin/env python3
"""
Simple Team Adjuster for Pokémon HGSS
------------------------------------
This script lets you adjust team sizes for specific trainers in a simpler way.

It can:
1. Set any trainer's team to a specific size
2. Makes sure the Pokémon count in both data files is updated
"""
...
def add_pokemon_to_trainer(rom, trainer_id, species_id, level, moves=None, append=True):
    """
    ...
    Args:
        ...
        append: If True, add to the end; if False, replace the team
    ...
    try:
        ...
        if not append:
            # Clear existing Pokémon if not appending
            pokemon_list = []
...
def main():
    ...
    if len(sys.argv) < 3:
        print(__doc__)
        print("\nUsage:")
        print("  python simple_team_adjuster.py boss <rom_file> [team_size]")
        print("  python simple_team_adjuster.py trainer <rom_file> <trainer_id> <team_size>")
        sys.exit(1)
    ...
    if command == "boss":
        ...
        # Set boss teams
        modified = max_team_size_bosses(rom, target_size=team_size)
    ...
    # Save ROM
    output_path = rom_path.replace(".nds", "_teams_adjusted.nds")
