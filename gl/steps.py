...
def select_cosmetic_variant(context, mondata, base_species_id, decision_path):
    ...
    Automatically filters out any cosmetic forms that are on the blacklist.
    """
    
    # Get blacklist to filter cosmetic variants
    blacklist = context.get(LoadBlacklistStep)
    blacklist_ids = blacklist.by_id if blacklist else set()
    ...
    # Filter out blacklisted cosmetic variants
    cosmetic_variants = [vid for vid in cosmetic_variants if vid not in blacklist_ids]
    
    # If no variants left after filtering (all blacklisted), return the originally selected species
    if len(cosmetic_variants) == 0:
        return mondata.data[base_species_id]
    
    # If only one variant available, return it directly
    if len(cosmetic_variants) == 1:
        return mondata.data[cosmetic_variants[0]]
...
class UpdateStaticOverworldSprites(Step):
    ...
    def run(self, context):
        ...
        mapping_path = 'gl/static_overworld_mapping.json'
